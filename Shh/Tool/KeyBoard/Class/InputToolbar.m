...
#define customKeyboardHeight self.frame.size.height+134
#define InputToolbarHeight self.frame.size.height
...
- (VoiceButtonView *)voiceButtonView
{
    if (!_voiceButtonView) {
        ...
        _voiceButtonView.backgroundColor = DSColorFromHex(0xFAFAFA);
    }
...
- (EmojiButtonView *)emojiButtonView
{
    if (!_emojiButtonView) {
        ...
        _keyboardHeight = customKeyboardHeight;
        _emojiButtonView.backgroundColor = DSColorFromHex(0xFAFAFA);
    }
...
- (MoreButtonView *)moreButtonView
{
    if (!_moreButtonView) {
        ...
        _keyboardHeight = customKeyboardHeight;
        _moreButtonView.backgroundColor = DSColorFromHex(0xFAFAFA);
    }
...
//        [self.voiceButton setImage:[UIImage imageNamed:@"voice_chat"] forState:UIControlStateNormal];
//        [self.voiceButton setImage:[UIImage imageNamed:@"voice_chat"] forState:UIControlStateHighlighted];
...
//    }
    ...
    if (!self.textInput) {
        self.textInput = [[UITextView alloc] initWithFrame:CGRectMake(15, 10, SCREEN_WIDTH - 90, 30)];
        self.textInput.font = [UIFont systemFontOfSize:18];
        self.textInput.layer.cornerRadius = 15;
        self.textInput.layer.masksToBounds = YES;
        self.textInput.backgroundColor = DSColorFromHex(0xF0F0F0);
    ...
    if (!self.emojiButton) {
        self.emojiButton = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.textInput.frame) + 5, 8, 30, 30)];
        [self.emojiButton setImage:[UIImage imageNamed:@"zan_big"] forState:UIControlStateNormal];
        [self.emojiButton setImage:[UIImage imageNamed:@"zan_selected"] forState:UIControlStateSelected];
...
//    }else{
//        image = @"expression_chat";
//        highImage = @"expression_chat";
//    }
//
//    // 设置图片
